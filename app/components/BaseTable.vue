...
<template>
  <div class="flex flex-col flex-1 w-full">
    ...
    <UTable ref="table" :column-filters="columnFilters" :data="[...data]" :columns="finalColumns" :loading="loading"
      :pagination="pagination" class="flex-1" />

    <div
      class="sticky bottom-0 flex items-center justify-center md:justify-end border-t border-default pt-4 px-4 bg-white/50 backdrop-blur-sm z-10 space-x-2">
      <USelect v-model="pagination.pageSize" :items="[10, 20, 50, 100]" size="sm"
        @update:model-value="updatePageSize" />
      <UPagination :model-value="pagination.pageIndex + 1" :items-per-page="pagination.pageSize"
        :total="pagination.total" size="sm" :sibling-count="0" :show-edges="showPaginationEdges"
